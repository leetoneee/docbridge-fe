...
import { Transaction, TransactionFilterParams } from '../../inbox/models/transaction.model';
...
export class OutboxApiService {
  ...
  create(payload: CreateOutboxPayload): Observable<Transaction> {
    return this.http.post<ApiResponse<Transaction>>(this.base, payload).pipe(map((r) => r.data!));
  }

  cancel(transactionCode: string, version: number, reason: string): Observable<string> {
    return this.http
      .patch<ApiResponse<string>>(`${this.base}/${transactionCode}/cancel`, { version, reason })
      .pipe(map((r) => r.data!));
  }
}
